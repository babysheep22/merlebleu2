        ...
        // 검색어가 포함된 아이템만 표시
        const tableRows = document.querySelectorAll('#top-results tbody tr');
        if (keyword) {
...
function myFunction() {
    const hiddenRows = document.querySelectorAll('#top-results tbody.hidden');
...
function getProductsFromHTML(includeHidden = false) {
    const tbodyElements = document.querySelectorAll('#top-results tbody');
    const products = [];
...
function renderProducts(products) {
    const productContainer = document.querySelector('#top-results');
