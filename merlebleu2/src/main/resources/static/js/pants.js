        ...
        // 검색어가 포함된 아이템만 표시
        const tableRows = document.querySelectorAll('#pants-results tbody tr');
        if (keyword) {
...
function myFunction() {
    const hiddenRows = document.querySelectorAll('#pants-results tbody.hidden');
...
function getProductsFromHTML(includeHidden = false) {
    const tbodyElements = document.querySelectorAll('#pants-results tbody');
    const products = [];
...
function renderProducts(products) {
    const productContainer = document.querySelector('#pants-results');
